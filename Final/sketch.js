...
function setup() {
  ...
  button.position(width * 0.4, height - height + 5);
...
function draw() {
  ...
  for (let i = 0; i < asteroids.length; i++) {
    ...
    if (asteroids[i].collide(spaceship)) {
      ...
      image(
        sparks,
        spaceship.x - width * 0.1,
        spaceship.y - height * 0.08,
        width / 4,
        height / 3
      );
    ...
    if (asteroids[i].died) {
      ...
      if (asteroids[i].size >= width / 20) {
        for (let k = 0; k < random(2, 4); k++) {
          asteroids.push(
            new Asteroid(asteroids[i].x, asteroids[i].y, asteroids[i].size / 2)
          );
        }
      }
  ...
  noStroke();
  textSize(width * 0.02);
  text("Kills   " + kills, width * 0.8, height * 0.04);
